...
function exibirDados(valor1: Array<{ id: number; nome: string; data: string; imagem?: string }>) {
...
function mudar(item: { id: number; nome: string; data: string; imagem?: string }) {
    const nomeInput = document.getElementById("Modalnome") as HTMLInputElement;
    const dataInput = document.getElementById("Modaldata") as HTMLInputElement;
    const usuInput = document.getElementById("Modalid_usu") as HTMLInputElement;
...
function troca() {
    const input_actual = document.getElementById("Modalnome") as HTMLInputElement;
    const input_data_actual = document.getElementById("Modaldata") as HTMLInputElement;
    const input_nova_imagem = document.getElementById("Modalficheiro") as HTMLInputElement;
    const input_id = document.getElementById("Modalid_usu") as HTMLInputElement;
    ...
    if (index !== -1) {
        ...
        if (nova_imagem) {
            ...
            reader.onloadend = function () {
                valor1[index].imagem = reader.result as string;
                atualizarItem();
            };
